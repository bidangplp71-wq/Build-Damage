...
export const BuildingPhotoGallery: React.FC<BuildingPhotoGalleryProps> = ({
  ...
  if (!photos || photos.length === 0) {
    if (googleDriveFolderUrl) {
      return (
        <div className="p-5 text-center rounded-2xl border border-indigo-200 bg-indigo-50/70 text-indigo-900 space-y-3">
          <Folder className="w-8 h-8 mx-auto text-indigo-600 opacity-90" />
          <div className="max-w-md mx-auto space-y-1">
            <p className="text-xs font-bold text-indigo-950">Dokumentasi Foto Tersimpan di Google Drive</p>
            <p className="text-[11px] text-indigo-700 leading-relaxed">
              Arsip foto visual gedung ini tersimpan secara terpusat di folder Google Drive Cloud Storage.
            </p>
          </div>
          <a
          ...
          >
            ...
            <span>Lihat Folder Foto di Google Drive ↗</span>
          </a>
        </div>
      );
    }
    ...
    return (
      <div className="p-6 text-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/50 text-slate-400">
        ...
        <p className="text-[11px] text-slate-400 mt-0.5">Maksimal 20 foto kerusakan dengan keterangan bagian yang difoto.</p>
      </div>
    );
  }

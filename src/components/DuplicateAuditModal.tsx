...
import { DuplicateGroup, isArchiveSource } from '../utils/duplicateDetector';
...
interface DuplicateAuditModalProps {
  ...
  onViewDetail?: (item: BuildingAssessment) => void;
}
...
export const DuplicateAuditModal: React.FC<DuplicateAuditModalProps> = ({
  ...
  const handleKeepAsMaster = (itemToKeep: BuildingAssessment) => {
    if (!currentGroup) return;
    const others = currentGroup.items.filter((x) => x.id !== itemToKeep.id);
    others.forEach((other) => {
      if (onDeleteAssessment) {
        onDeleteAssessment(other.id);
      } else {
        deleteAssessment(other.id);
      }
    });
    showToast(
      `✓ Data "${itemToKeep.buildingName}" berhasil dipertahankan sebagai Master. ${others.length} rekaman duplikat lainnya telah dibersihkan!`,
      'success'
    );
  };
  ...
  const handleIgnoreGroup = () => {
    ...
    showToast('Kelompok data ini berhasil ditandai sebagai data sah (Bukan Duplikat).', 'success');
    if (activeGroupIndex >= duplicateGroups.length - 1) {
      setActiveGroupIndex(Math.max(0, duplicateGroups.length - 2));
    }
  };

  const handleIgnoreSingleItem = (item: BuildingAssessment) => {
    if (!onIgnorePair) return;
    onIgnorePair(item.id);
    showToast(`Data "${item.buildingName}" ditandai sebagai data sah.`, 'success');
  };
                        ...
                        >
                              ...
                              <div>
                                ...
                                <div className="mt-1 flex flex-wrap items-center gap-1.5">
                                  {isArchiveSource(item.sourceSheet) ? (
                                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-amber-100 text-amber-950 border border-amber-300 text-[11px] font-extrabold shadow-2xs">
                                      <Folder className="w-3 h-3 text-amber-700 shrink-0" />
                                      <span>Asal: Arsip ({item.sourceSheet})</span>
                                      <span className="bg-amber-200 text-amber-900 px-1 rounded text-[9px] font-mono">Read-Only</span>
                                      {item.sheetRowNumber && (
                                        <span className="font-mono text-amber-900 bg-amber-200/80 px-1 py-0.2 rounded">
                                          Baris #{item.sheetRowNumber}
                                        </span>
                                      )}
                                    </span>
                                  ) : (
                                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-blue-50 text-blue-900 border border-blue-200 text-[11px] font-bold">
                                      ...
                                      <span>Asal Tab: {item.sourceSheet || (item.kecamatanName ? `Kec. ${item.kecamatanName}` : 'Kecamatan')}</span>
                                      {item.sheetRowNumber && (
                                        <span className="font-mono text-blue-700 bg-blue-100/70 px-1 py-0.2 rounded">
                                          Baris #{item.sheetRowNumber}
                                        </span>
                                      )}
                                    </span>
                          ...
                          <div className="pt-3 border-t border-slate-200 flex flex-wrap items-center gap-2">
                            ...
                            >
                              ...
                              <span>Tandai Sah</span>
                            </button>

                            {(currentUser.role === 'super_admin' || currentUser.role === 'admin') && currentGroup && currentGroup.items.length > 1 && (
                              <button
                                onClick={() => handleKeepAsMaster(item)}
                                className="w-full py-2 px-3 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl border border-emerald-700 transition-colors flex items-center justify-center gap-1.5 shadow-2xs cursor-pointer mt-1"
                                title="Jadikan data ini sebagai Master yang sah dan hapus duplikat lainnya dari arsip/ganda"
                              >
                                <CheckCircle className="w-3.5 h-3.5" />
                                <span>Simpan Ini (Hapus Duplikat Lain)</span>
                              </button>
                            )}

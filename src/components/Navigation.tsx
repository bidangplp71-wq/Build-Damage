...
export const Navigation: React.FC<NavigationProps> = ({ mobileOpen, onCloseMobile }) => {
  ...
  const tabs = [
    ...
    },
    {
      ...
      desc: 'Tersimpan otomatis ke link sheet',
  ...
  const navContent = (
    <div className="flex flex-col h-full bg-slate-900 text-slate-100 border-r border-slate-800">
      ...
      {/* Role Switching & System Status Card at Bottom of Vertical Tab Rail */}
      <div className="p-3 border-t border-slate-800 bg-slate-950/50 space-y-2">
        <div className="relative">
          <button
            onClick={() => setRoleMenuOpen(!roleMenuOpen)}
            className="w-full flex items-center justify-between p-2.5 rounded-xl bg-slate-800/90 hover:bg-slate-800 border border-slate-700/80 transition-colors text-left"
          >
            <div className="flex items-center gap-2.5 min-w-0">
          ...
          {roleMenuOpen && (
            <div className="absolute bottom-full left-0 right-0 mb-2 rounded-xl bg-slate-800 border border-slate-700 shadow-2xl p-2 z-50 animate-in fade-in zoom-in-95 duration-100">
              <div className="px-2.5 py-1.5 border-b border-slate-700/80 mb-1.5">
                ...
                <p className="text-[10px] text-slate-400">Pilih role untuk uji coba kuota & hak akses:</p>
              </div>
